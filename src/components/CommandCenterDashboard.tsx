...
export default function DashboardCommandCenter({
  ...
  const mainInsightFocus = totals.mainInsight.sourceData || {};
  const highlightedCategoryName =
    String(mainInsightFocus.categoryName || mainInsightFocus.category || "").trim();
  const highlightedWalletName =
    String(mainInsightFocus.walletName || mainInsightFocus.cardName || "").trim();
  const highlightedSubscriptionName = String(
    mainInsightFocus.subscriptionName || mainInsightFocus.serviceName || ""
  ).trim();
  const priorityBadgeClass =
    totals.mainInsight.priority === "CRITICAL"
      ? "border-rose-400/20 bg-rose-500/10 text-rose-100"
      : totals.mainInsight.priority === "HIGH"
      ? "border-amber-400/20 bg-amber-500/10 text-amber-100"
      : totals.mainInsight.priority === "MEDIUM"
    ...
    <div className={shellClass} data-testid={TEST_IDS.dashboardPageDesktop}>
      <section className="card-premium rounded-[32px] p-5 lg:p-6">
          ...
          <div className="space-y-4 xl:col-span-4">
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span
                  className={`rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] ${priorityBadgeClass}`}
                >
                  {totals.mainInsight.priority}
                </span>
                <span
                ...
                >
                  {totals.mainInsight.type.replaceAll("_", " ")}
                </span>
              </div>

              {totals.mainInsight.impactEstimate ? (
                <p
                  className={`mt-3 text-xs leading-5 ${
                    isLight ? "text-slate-500" : "text-slate-400"
                  }`}
                >
                  Impacto estimado: {totals.mainInsight.impactEstimate}
                </p>
              ) : null}

              {highlightedCategoryName || highlightedWalletName ? (
              ...
              {totals.mainInsight.actionSuggestion ? (
                <p
                  className={`mt-3 text-xs font-semibold ${
                    isLight ? "text-slate-500" : "text-slate-400"
                  }`}
                >
                  CTA sugerido: {totals.mainInsight.actionSuggestion}
                </p>
              ) : null}
                ...
                {topWallets.map((wallet) => (
                  <div
                    ...
                    className={`flex items-center justify-between gap-3 rounded-2xl border px-3 py-2.5 ${
                      ...
                      highlightedWalletName &&
                      wallet.name.toLowerCase() === highlightedWalletName.toLowerCase()
                        ? "ring-2 ring-indigo-400/40 shadow-[0_0_0_1px_rgba(99,102,241,0.18),0_0_24px_rgba(99,102,241,0.2)]"
                        : ""
                    }`}
                  >
        ...
        <div className={compact ? "mt-4 space-y-4" : "mt-4 grid gap-4 xl:grid-cols-12"}>
          <div
            id="anomaly-analysis-chart"
            className={getSectionClass(
              "anomaly-analysis-chart",
              `xl:col-span-7 rounded-[28px] border p-5 ${isLight ? "border-slate-200 bg-white" : "border-white/10 bg-white/5"}`
            )}
          >
          ...
          <div id="credit-utilization-chart" className="scroll-mt-28" aria-hidden="true" />
            <div
            ...
            <div className="mt-4 space-y-2">
              {subscriptionOverview.items.slice(0, 3).map((item) => (
                <div
                  key={item.id}
                  className={`rounded-2xl border px-3 py-2.5 transition ${isLight ? "border-slate-200 bg-white" : "border-white/10 bg-white/5"} ${
                    highlightedSubscriptionName &&
                    item.displayName.toLowerCase().includes(highlightedSubscriptionName.toLowerCase())
                      ? "ring-2 ring-indigo-400/35 shadow-[0_0_0_1px_rgba(99,102,241,0.16),0_0_24px_rgba(99,102,241,0.18)]"
                      : ""
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
        ...
        <div id="subscriptions-chart" className="scroll-mt-28" aria-hidden="true" />
        {showAdvanced ? (
